...
export const ClockWidget = ({ config }) => {
        ...
        return (
            <div className="clock-analog" style={{
                ...
                <div style={{
                    ...
                    {[...Array(12)].map((_, i) => (
                        <div key={i} style={{
                            position: 'absolute', width: '100%', height: '100%', left: '0', top: '0', transform: `rotate(${i * 30}deg)`
                        }}>
                            <div style={{
                                margin: '4% auto 0',
                                width: i % 3 === 0 ? '2.5px' : '1px',
                                height: '6%',
                                background: i % 3 === 0 ? 'white' : 'rgba(255,255,255,0.4)',
                                borderRadius: `calc(1px * var(--radius-scale, 1))`
                            }} />
                        </div>
                    ))}

                    {/* Hands */}
                    <div style={{ position: 'absolute', width: '4px', height: '25%', background: 'white', bottom: '50%', left: '50%', transformOrigin: 'bottom', borderRadius: `calc(4px * var(--radius-scale, 1))`, transform: `translateX(-50%) rotate(${hours}deg)`, transition: 'transform 0.5s cubic-bezier(0.4, 2.08, 0.55, 0.44)' }} />
                    <div style={{ position: 'absolute', width: '3px', height: '35%', background: 'rgba(255,255,255,0.7)', bottom: '50%', left: '50%', transformOrigin: 'bottom', borderRadius: `calc(3px * var(--radius-scale, 1))`, transform: `translateX(-50%) rotate(${minutes}deg)`, transition: 'transform 0.5s cubic-bezier(0.4, 2.08, 0.55, 0.44)' }} />
                    <div style={{ position: 'absolute', width: '1.5px', height: '40%', background: 'var(--accent-primary)', bottom: '50%', left: '50%', transformOrigin: 'bottom', borderRadius: `calc(2px * var(--radius-scale, 1))`, transform: `translateX(-50%) rotate(${seconds}deg)`, transition: seconds === 0 ? 'none' : 'transform 0.2s cubic-bezier(0.4, 2.08, 0.55, 0.44)' }} />
    ...
    return (
        ...
        >
            <div style={{ fontSize: 'clamp(1.5rem, 25cqmin, 6rem)', fontWeight: '800', letterSpacing: '-1cqmin', lineHeight: '1' }}>
                {timeString}
            </div>
            <div style={{ fontSize: 'clamp(0.6rem, 10cqmin, 2rem)', fontWeight: '500', opacity: 0.6, marginTop: '2cqmin', textTransform: 'uppercase', letterSpacing: '0.1cqmin' }}>
                {dateString}
            </div>
        </div>
    );
